...
## This repo used for refactoring the project 1 from CMPUT291 
...
See: https://docs.python.org/3/library/hashlib.html for documentation.
...
At development time you will be testing your programs with your own data sets (make sure that it conforms to the project specification). At demo time we will be creating the database using the SQL statements for creating the tables mentioned above, and we will be populating it with our own test data set. Your application will be tested under a demo account (i.e., not your own account).

The demo will be run using the source code that you submitted and nothing else. Make sure your submission includes every file that is required to run your application. You will neither be able to change your code, nor use any file other than those submitted. This policy can only be altered in exceptional cases at the instructor's discretion and for a hefty penalty. As a test drill, you should be able to set up your application under someone else's account within 3 minutes at most.

Every group will book a time slot convenient to all group members to demo their projects during one of the lab hours. At demo time, all group members must be present. The TA will be using a script to both create and populate the tables. The TA will be asking you to perform various tasks to demonstrate how your application is handling the specifications above. The TA will mark your demo immediately after the testing.
...
To submit your source code do the following:
...
Upload your project tarfile to eClass on the assignment page.
(2) Design document.

Your design document must be typewritten and submitted as a hardcopy at the designated drop box located on the first floor of the CSC building, across from room CSC 1-45, before the due date. Your design document must not exceed 4 pages.

The design document should include the following:

(a) General overview of your system with a short user guide.
...
The detailed component design of your software should describe how each of the components delivers the major functions of your application, including the responsibility and interface of each primary class and the structure and relationships among them. (From this description it follows that you must have an object-oriented design of your system.)

(c) Testing strategy.

The testing strategy discusses your general strategy for testing, including the scenarios that you have tested, the coverage of your test cases and (if applicable) some statistics on the number of bugs found and the nature of those bugs.

(d) Group work description.

The group work description must list the how the work was distributed among group partners, must indicate the time spent (an estimate), must explain the progress made by each partner, and must describe your method of coordination to keep the project on track.

(e) Other

The design document should also include a documentation of any decisions you have made, which are not in the project specification, as well as any coding you have done beyond or different from what is required.

Your design document must not include the source code. However your source code (which is submitted electronically) will be inspected for source code quality (whether the code is easy to read, properly documented, meaningful names have been given to classes, methods, functions, and variables, and whether the data processing is all done in SQL!).
